...
export default class UserController {
  ...
  public async generateOtp(request: WRRequest<CreateUser, undefined>, response: Response) {
    const valSchema = new User().getGenerateOtpVS();
    const result = valSchema.validate(request.body);
    if (result.error == null) {
      const resp = await this.service.generateOtp(request.body);
